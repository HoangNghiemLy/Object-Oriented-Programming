...
public class EmployeeList {
	...
	public Map<Integer, Long> getNoOfEmployeesByYOB(){
		Map<Integer, Long> countByYOB = new HashMap();
		int currentYear = Calendar.getInstance().get(Calendar.YEAR);
		for(Employee emp : employees) {
			LocalDate dob = emp.getDob();
			int yearOfBirth = dob.getYear() + 1900;
			//Đếm số lượng nhân viên theo năm sinh
			countByYOB.put(yearOfBirth, countByYOB.getOrDefault(yearOfBirth, 0L)+1);
		}
		return countByYOB;
	}
}
