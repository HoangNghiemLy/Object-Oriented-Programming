...
public class QuanLyChuyenXe {
	...
	/*
	 * Cập nhật chuyến xe khi biết mã chuyến xe
	 */
	public boolean capNhatChuyenXe(ChuyenXe capNhatMoi) {
		int i = this.dscx.indexOf(timKiemChuyenXeTheoMa(capNhatMoi.getMaSoChuyenXe()));
		if(i<0)
			return false;
		this.dscx.set(i, capNhatMoi);
		return true;
	}
	/*
	 * Thống kê doanh thu trung bình cua all chuyến xe
	 */
	public Map<String, Double> thongKeDoanhThuTrungBinhCuaTatCaChuyenXe(){
		Map<String, Double> tmp =this.dscx.stream().collect(Collectors.groupingBy(x->x.getClass().toString(),Collectors.averagingDouble(ChuyenXe::getDoanhThu)));
		return tmp;
	}
}
